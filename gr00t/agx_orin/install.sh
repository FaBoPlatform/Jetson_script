...
conda create -y -n model python=3.10
...
# ===== conda パッケージ =====
conda install -y -c conda-forge libpng 
...
cd ffmpeg-4.4
./configure --prefix=/usr/local/ffmpeg-4.4 --enable-shared --disable-static
make -j$(nproc)
sudo make install
echo "/usr/local/ffmpeg-4.4/lib" | sudo tee /etc/ld.so.conf.d/ffmpeg-4.4.conf
sudo ldconfig
ffmpeg -version
ldd $(which ffmpeg) | grep ffmpeg-4.4
...
LD_PRELOAD="/usr/local/ffmpeg-4.4/lib/libavutil.so.56 /usr/local/ffmpeg-4.4/lib/libavfilter.so.7" \
LD_LIBRARY_PATH=/usr/local/ffmpeg-4.4/lib \
python ./scripts/gr00t_finetune.py --help

LD_PRELOAD="/usr/local/ffmpeg-4.4/lib/libavutil.so.56 /usr/local/ffmpeg-4.4/lib/libavfilter.so.7" \
LD_LIBRARY_PATH=/usr/local/ffmpeg-4.4/lib \
 python ./scripts/gr00t_finetune.py \
   --dataset-path ./demo_data/so100_strawberry_grape/ \
   --num-gpus 1 \
